...
class HBT_Profit_Calculator {
    ...
    public function calculate_order( int $order_id ): bool {
       ...
       // Determine total commission for this order (sum of relevant transactions).
        ...
        // ÇÖZÜM UYGULANDI: Veritabanına doğrudan SQL atmak yerine zaten yukarıda sorguladığımız $transactions dizisini sayıyoruz.
        // Bu sayede olmayan tablo yüzünden çıkan 500 Hatası kalıcı olarak önlenmiş oldu.
        $tx_exists = count( $transactions );
  ...
  /**
    ...
    public function recalculate_all( ?int $store_id = null ): void {
        ...
        // 1. ZAMAN AŞIMINI (500 Hatasını) KESİN ÖNLEME
        ...
        // 2. SABİT GİDERLERİN ESKİ SİPARİŞLERE YANSIMAMA SORUNUNUN ÇÖZÜMÜ
        // Eğer kullanıcı Sistem Onarım panelinden "Tümünü Hesapla" butonuna bastıysa, 
        // önceki tüm hesaplamaları zorla sıfırla ki Sabit Giderler eski siparişlere de yansısın!
        if ( isset( $_POST['order_id'] ) && $_POST['order_id'] === 'all' ) {
            $update_sql = "UPDATE {$wpdb->prefix}hbt_orders SET is_calculated = 0";
            if ( $store_id !== null ) {
                $wpdb->query( $wpdb->prepare( $update_sql . " WHERE store_id = %d", $store_id ) );
            } else {
        ...
        // İşlem bitince WP Cache'i tekrar aktif et
