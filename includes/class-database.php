...
class HBT_Database {
   ...
   /** Dashboard grafiği için son 30 günlük trend (Türkiye Saati) */
	...
	public function log_sync_result( array $data ): int {
		...
		// YENİ: Zaman dilimini kesin olarak Türkiye saatine (Europe/Istanbul) ayarla
		$tz = new DateTimeZone( 'Europe/Istanbul' );
		$dt = new DateTime( 'now', $tz );
		$data['created_at'] = $dt->format( 'Y-m-d H:i:s' );

		// Veritabanına logu ekle
		$this->wpdb->insert( $table, $data );
		$insert_id = $this->wpdb->insert_id;

		// --- OTOMATİK TEMİZLEME: SADECE SON 100 LOG KALSIN ---
		// 100. sıradaki logun ID'sini bul
		$hundredth_id = $this->wpdb->get_var( "SELECT id FROM {$table} ORDER BY id DESC LIMIT 1 OFFSET 99" );
		
		// Eğer 100'den fazla log varsa, bu ID'den küçük (eski) olanların hepsini sil
		if ( $hundredth_id ) {
			$this->wpdb->query( $this->wpdb->prepare( "DELETE FROM {$table} WHERE id < %d", $hundredth_id ) );
		}
		// -----------------------------------------------------
