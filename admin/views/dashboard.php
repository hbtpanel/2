...
jQuery(document).ready(function($) {
    ...
    $.post(hbtTpt.ajaxurl, { action: 'hbt_get_dashboard_data', nonce: hbtTpt.nonce }, function(res) {
      ...
      // Mağaza Bazlı Rapor ve Ciro Tablosu
        ...
        // PHP'den revenue verisinin geldiğini varsayıyoruz (Daha önce SQL'de ayarladığımız gibi)
        ...
        $.each(storesMap, function(id, st) {
            ...
            // Düne kıyasla ciro değişim yüzdesinin hesaplanması
            var percentChangeStr = '';
            if (st.rev_yesterday > 0) {
                var change = ((st.rev_today - st.rev_yesterday) / st.rev_yesterday) * 100;
                var color = change >= 0 ? '#10B981' : '#EF4444'; // Artış yeşil, düşüş kırmızı
                var sign = change > 0 ? '+' : '';
                ...
                // Dün satış yok, bugün varsa %100 artış kabul edilir
                percentChangeStr = ' <span style="color:#10B981; font-size:12px; font-weight:bold; margin-left:6px;">(+100%)</span>';
            }

            // Ciro Tablosu Satırları (FormatMoney 'true' yapılarak kalınlık ve renk eşitlendi)
            tbodyRevenue += '<tr>';
